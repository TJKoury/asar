...
module.exports.readArchiveHeaderSync = function (archive) {
  ...
  let size
  let headerBuf
  this.checkFileSignature(archive);
  try {
...
module.exports.checkFileSignature = function (archive, position) {
  const fd = fs.openSync(archive, 'r')
  let endCapSize = 8;
  ...
  if (size > endCapSize) {
    ...
    console.log("checkFileSignature", _b.readUIntLE(0, 6));
    console.log("checkFileSignature", _b.readInt16LE(6, 2));
  }
